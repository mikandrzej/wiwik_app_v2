...
void EgRestDataSource::onVehicleListRequestReady(QByteArray& replyData)
{
    ...
    foreach (auto jsonVeh, vehArray)
    {
        ...
        qDebug() << "Parsed vehicle data: " << vehData->id << vehData->name << vehData->plateNo << vehData->color;
...
void EgRestDataSource::onDevicesListRequestReady(QByteArray& replyData)
{
    ...
    {
        newDevList.append(new Device(dev->id, dev->serial_no, dev->name, dev->vehicle_id, dev->type));
    }
...
void EgRestDataSource::onSensorsListRequestReady(QByteArray& replyData)
{
    ...
    foreach (auto jsonSens, sensArray)
    {
        ...
        qDebug() << "Parsed sensor data: " << sensData->id << " " << sensData->name << " " << sensData->type << " " << sensData->address << " " << sensData->device_id
                 << sensData->device_name;
    ...
    {
        newSensorList.append(new Sensor(sensor->id, sensor->name, sensor->address, sensor->type, sensor->device_id));
    }
