...
FormLiveView::FormLiveView(QWidget *parent)
    : QWidget(parent)
    , ui(new Ui::FormLiveView)
{
    ui->setupUi(this);
    ...
    for (auto sensor : DataContainer::instance()->getSensorsList()) {
        if (sensor->type() == "gps") {
            connect(sensor, &Sensor::lastMeasureChanged, this, [this, sensor]() {
                sensorPositionDataChanged(sensor);
            });
        }
    }

    connect(DataContainer::instance(),
            &DataContainer::sensorDataInsertDone,
            this,
            [this](int newIndex) {
                ...
                if (newSensor->type() == "gps") {
                    connect(newSensor, &Sensor::lastMeasureChanged, this, [this, newSensor]() {
                        sensorPositionDataChanged(newSensor);
                    });
                }
            });
}
