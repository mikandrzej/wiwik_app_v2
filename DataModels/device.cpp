...
Device::Device(int id, QString& serialNo, QString& name, int vehicleId, QString& type, QObject* /*parent*/)
...
Device::Device(const Device& device, QObject* parent) :
    QObject {parent},
    m_id(device.id()),
    m_serialNo(device.serialNo()),
    m_name(device.name()),
    m_vehicleId(device.vehicleId()),
    m_type(device.type())
{
}
...
bool Device::changesPending() const
{
...
void Device::commitChanges()
{
    ...
    connect(reply, &QNetworkReply::finished, reply, [reply, this]() {
        ...
        if (status_code.isValid())
        {
            ...
            {
                qWarning() << "commit changes for device " << m_id << "error with code: " << status_code;
            }
        }
    });
}
