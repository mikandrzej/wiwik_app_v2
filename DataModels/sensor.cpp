...
bool Sensor::changesPending() const
{
...
void Sensor::commitChanges()
{
    ...
    connect(reply, &QNetworkReply::finished, reply, [reply, this]() {
        ...
        if (status_code.isValid())
        {
            ...
            {
                qWarning() << "commit changes for sensor " << m_id << "error with code: " << status_code;
            }
        }
    });
}

Measure* Sensor::lastMeasure() const
{
...
QMap<QDateTime, Measure*> Sensor::measures() const
{
    return m_measures;
}
