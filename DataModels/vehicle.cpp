...
Vehicle::Vehicle(int id, QString& name, QString& plateNo, QColor& color, QObject* parent) : QObject(parent), m_id(id), m_name(name), m_plateNo(plateNo), m_color(color) {}

Vehicle::Vehicle(const Vehicle& vehicle, QObject* parent) : QObject(parent), m_id(vehicle.id()), m_name(vehicle.name()), m_plateNo(vehicle.plateNo()), m_color(vehicle.color())
{
}
...
void Vehicle::commitChanges()
{
    ...
    connect(reply, &QNetworkReply::finished, reply, [reply, this]() {
        ...
        if (status_code.isValid())
        {
            ...
            {
                qWarning() << "commit changes for vehicle " << m_id << "error with code: " << status_code;
            }
        }
    });
}

bool Vehicle::changesPending() const
{
    return m_changesPending;
}
