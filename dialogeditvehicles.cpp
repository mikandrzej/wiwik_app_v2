...
DialogEditVehicles::DialogEditVehicles(QWidget* parent) : QDialog(parent), ui(new Ui::DialogEditVehicles)
{
    ui->setupUi(this);

    connect(ui->lv_vehicles, &QAbstractItemView::clicked, this, &DialogEditVehicles::onVehicleClicked);
}

DialogEditVehicles::~DialogEditVehicles()
{
    delete ui;
}
...
void DialogEditVehicles::onVehicleClicked(const QModelIndex& index)
{
    ...
    ui->le_vehName->setText(m_carListModel->data(index.siblingAtColumn(VehiclesEditModel::ColumnName), Qt::DisplayRole).toString());
    ui->le_plateNo->setText(m_carListModel->data(index.siblingAtColumn(VehiclesEditModel::ColumnPlateNo), Qt::DisplayRole).toString());
}
...
void DialogEditVehicles::on_pb_saveVehicle_clicked()
{
    ...
    bool ok;
    auto id = m_carListModel->data(m_clickedIndex.siblingAtColumn(VehiclesEditModel::ColumnId), Qt::DisplayRole).toInt(&ok);
    if (!ok)
    {
