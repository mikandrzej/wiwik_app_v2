...
{
    connect(DataContainer::instance(), &DataContainer::deviceDataToBeInserted, this, [this](int row) { beginInsertRows(QModelIndex(), row, row); });
    connect(DataContainer::instance(), &DataContainer::deviceDataInsertDone, this, [this]() { endInsertRows(); });
    connect(DataContainer::instance(), &DataContainer::deviceDataChanged, this, [this](int idx) {
...
QVariant DeviceTableModel::data(const QModelIndex& index, int role) const
{
    ...
    if (Qt::DisplayRole == role || Qt::EditRole == role)
    {
        switch (static_cast<Columns>(column))
        {
            ...
            case ColumnVehicle:
            {
                const auto* vehicle = DataContainer::instance()->getVehicleById(device->vehicleId());
                if (vehicle != nullptr)
