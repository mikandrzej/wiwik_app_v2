...
EgMqttDataSource::EgMqttDataSource(QObject* parent) : QMqttClient {parent}
{
    ...
    connect(this, &QMqttClient::stateChanged, this, &EgMqttDataSource::onClentStateChanged);
}
...
void EgMqttDataSource::onClentStateChanged(QMqttClient::ClientState state)
{
    if (Connected == state)
    {
        ...
        connect(subsVeh, &QMqttSubscription::messageReceived, this, &EgMqttDataSource::onVehiclesMessageReceived);

        auto subsServer = subscribe(QMqttTopicFilter("server/#"));
        connect(subsServer, &QMqttSubscription::messageReceived, this, &EgMqttDataSource::onServerMessageReceived);
    }
...
{
    qDebug() << "Received MQTT message. Topic: " + msg.topic().name() + " msg " + msg.payload();

    QStringList topicLevels = msg.topic().name().split('/');

    if (topicLevels[0] != "vehicles")
    {
        qWarning() << "Invalid vehicle mqtt message topic level[0]. Received: " << msg.topic().name();
        return;
    }
...
void EgMqttDataSource::onServerMessageReceived(QMqttMessage msg)
{
    ...
    {
        qWarning() << "Invalid server mqtt message topic level[0]. Received: " << msg.topic().name();
        return;
    }
...
void EgMqttDataSource::parseServiceServerMesssage(QString& parameter, const QByteArray& message)
{
    if (parameter == "uptime")
    {
        bool ok;
...
void EgMqttDataSource::parseVehicleMessage(int vehicle_id, QStringList& topicLevels, const QByteArray& message)
{
