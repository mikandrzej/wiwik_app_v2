        ...
        case ColumnId:
            switch (role)
            {
                ...
                case RoleMarkerPosition:
                    return QVariant::fromValue(item->positionModel()->lastValue().coordinate());
                case RoleMarkerType:
                    return "marker";
        ...
        case ColumnPosition:
            switch (role)
            {
                case Qt::DisplayRole:
                {
                    ...
                    if (position.isValid())
                    {
                        return QString("Lat:%1 Long:%2").arg(QString::number(position.coordinate().latitude()), QString::number(position.coordinate().longitude()));
                    }
                    break;
                }
                case ColumnBattery:
...
void VehiclesModel::onSensorDataReceived(EgVehicleSensorData& sensorData)
{
    ...
    switch (sensorData.dataType)
    {
        case EgSensorDataType::Temperature:
            model->temperatureModel()->insert(sensorData.timestamp.toSecsSinceEpoch(), sensorData.temperature);
            emit dataChanged(index(row, ColumnTemperature), index(row, ColumnTemperature));
            break;
        case EgSensorDataType::BatteryVoltage:
            model->batteryModel()->insert(sensorData.timestamp.toSecsSinceEpoch(), sensorData.battery);
            emit dataChanged(index(row, ColumnBattery), index(row, ColumnBattery));
            break;
        case EgSensorDataType::GpsPosition:
            model->positionModel()->insert(sensorData.timestamp.toSecsSinceEpoch(), sensorData.geoPosition);
...
void VehiclesModel::onSensorsDataReceived(EgSensorsData& sensorsData)
{
    for (auto sensorData : sensorsData.sensors)
    {
        ...
        switch (sensorData->dataType)
        {
            ...
            case EgSensorDataType::GpsPosition:
                model->positionModel()->insert(sensorData->timestamp.toSecsSinceEpoch(), sensorData->geoPosition);
                emit dataChanged(index(m_vehicleRowByModel[model], ColumnPosition), index(m_vehicleRowByModel[model], ColumnPosition), {RoleMarkerPosition});
                break;
            default:
                break;
        }
